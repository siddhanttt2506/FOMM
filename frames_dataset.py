...
class FramesDataset(Dataset):
    ...
    def _getitem_(self, idx):
        while True:  # Keep trying until a valid video is found
            ...
            if self.is_train and os.path.isdir(path):
                ...
                num_frames = len(frames)
                if num_frames == 0:  # Skip empty folders
                    print(f"Skipping video {video_name} because it has no frames.")
                    idx = (idx + 1) % len(self.videos)  # Move to the next index
                    continue
                ...
                if video_array is None or len(video_array) == 0:  # Skip empty videos
                    print(f"Skipping video {video_name} because it has no frames.")
                    idx = (idx + 1) % len(self.videos)  # Move to the next index
                    continue
